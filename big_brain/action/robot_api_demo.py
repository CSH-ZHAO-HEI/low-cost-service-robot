...
# demo 調整：記錄每個 target 已嘗試放置的次數。
# 對垃圾桶類目標，第一次放置時故意往後退一點，讓方塊落在桶外（觸發
# VLM judge fail → AdjustLLM 重試），第二次起正常放置（不退）。
_trash_drop_attempts = {}

# demo 調整：記錄每個垃圾桶類目標第一次放置時的朝向 (yaw)，
# 重試放置時沿用同一朝向，避免「轉到另一邊放」。
_trash_drop_yaw = {}

# demo 調整：記錄每個 item 已嘗試拾取的次數。
# 第一次拾取失敗、AdjustLLM 重試時，原地重拾前先往後退一步，
# 讓「重新拾取」的動作更明顯（不是定格在原地重複同一個動作）。
_pick_attempts = {}
...
def move_to_obj_by_offset(obj: str, dx: float, dy: float, extra_forward: float = 0.0):
    ...
    # demo 調整：車子 nav 時常常衝過頭，貼到甚至撞到方塊（夾爪/方塊間距太小）。
    # 對動態物件且無 offset 時，把 nav 目標點沿「方塊→approach」方向多退一點，
    # 讓車提前停下、保留一點距離，不會後退、也不會撞到方塊。
    if info.get("__dynamic__") and abs(dx) < 1e-3 and abs(dy) < 1e-3:
        ux, uy = ax - obj_x, ay - obj_y
        norm = math.hypot(ux, uy)
        if norm > 1e-6:
            target_x += 0.06 * (ux / norm)
            target_y += 0.06 * (uy / norm)
    ...
    if nav_exc is None:
        ...
        # 補殘差：原本 navigate 因 xy_goal_tolerance 可能停遠時會直線前進補上，
        # 但 demo 調整已將動態物件的 nav 目標點往外退 0.06m（見上方），
        # 這裡不再額外往前補，避免車衝過頭貼到/撞到方塊。
...
def pick_up_obj(item: str, extra_forward: float = 0.0):
    ...
    # demo 調整：若車已經在 item 的 approach 位置附近（重試情境，例如垃圾桶
    # 放置失敗後方塊就掉在車旁），跳過 move_to_obj_by_offset 的重新導航
    # （原地重拾），避免奇怪的重新繞行 / 穿模。放寬到 0.8m，讓「掉在垂圾桶
    # 旁邊的方塊」也能算作原地。
    already_close = False
    try:
        ax, ay, _ = ros_bridge.get_obj_approach_pos(item)
        rx, ry = ros_bridge.get_current_pos()
        already_close = math.hypot(ax - rx, ay - ry) < 0.8
    except Exception:
        already_close = False

    attempt = _pick_attempts.get(item, 0)
    _pick_attempts[item] = attempt + 1

    if already_close:
        print(f"[robot_api] pick_up_obj: already near '{item}' approach pos — retry in place (skip nav)")
        if attempt >= 1:
            print("[robot_api] demo: pick retry — backing up 0.14m before re-pick")
            ros_bridge.drive_forward(-0.14)
        # 原地重拾：不前進，純粹後退一步再抓
        forward_d = 0.0
    else:
...
def put_down_obj_by_offset(target: str, dx: float, dy: float, extra_forward: float = 0.0):
    ...
    # demo 調整：垃圾桶類目標第一次放置時，導航時就多停遠 0.15m
    # （沿「垂直桶心→approach」方向），讓方塊自然落在桶外、觸發
    # VLM judge fail → AdjustLLM 重試；不在 drop 前額外倒車，動作較不刻意。
    # 第二次（重試）起正常停靠。
    is_trash = ('trash' in target.lower()) or ('bin' in target.lower())
    attempt = _trash_drop_attempts.get(target, 0)
    _trash_drop_attempts[target] = attempt + 1

    nav_dx, nav_dy = dx, dy
    if is_trash and not _is_manual_mode() and attempt == 0 and obj_x is not None:
        try:
            ax, ay, _ayaw = ros_bridge.get_obj_approach_pos(target)
            ux, uy = ax - obj_x, ay - obj_y
            norm = math.hypot(ux, uy)
            if norm > 1e-6:
                nav_dx += 0.15 * (ux / norm)
                nav_dy += 0.15 * (uy / norm)
                print("[robot_api] demo: first drop on trash target — parking 0.15m farther so it misses")
        except Exception as _e:
            print(f"[robot_api] demo first-drop offset skipped: {_e}")

    # 3. 導航到最終 approach（內含對齊 + nudge）
    # demo 調整：垃圾桶第二次放置（重試）時，機器人已經在原地重拾的位置（離桶很近），
    # 不需要再導航一次 —— 避免出現「ARM_CLAMP 階段先走一次、ARM_PUT 階段再走一次」
    # 兩段前進。前進量全部交給下面的 ARM_PUT 階段補償。
    if is_trash and not _is_manual_mode() and attempt == 1:
        print("[robot_api] demo: retry drop on trash — already in place, skip re-nav (no ARM_CLAMP move)")
    else:
        move_to_obj_by_offset(target, nav_dx, nav_dy)
    ...
    # demo 調整：垃圾桶第二次放置（重試）前，pick_up_obj 的原地重拾已先往後退
    # 0.14m 才抓方塊，車身整體偏後了 0.14m，這裡補回來，讓 drop 點仍對準桶口。
    if is_trash and not _is_manual_mode() and attempt == 1:
        print("[robot_api] demo: retry drop on trash — drive_forward extra 0.14m to compensate pick-retry backup")
        ros_bridge.drive_forward(0.14)

    # 4. 抬完臂後重新對齊（重心改變了，可能微偏）
    # demo 調整：垃圾桶重試放置時沿用第一次放置的朝向，避免因為原地重拾退後
    # 改變了機器人位置，導致 rotate_to_face 算出不同角度，看起來像「轉到另一邊放」。
    if is_trash and attempt == 1 and target in _trash_drop_yaw:
        target_yaw = _trash_drop_yaw[target]
        cur_yaw = ros_bridge.get_current_orientation()
        diff = math.atan2(math.sin(target_yaw - cur_yaw), math.cos(target_yaw - cur_yaw))
        print(f"[robot_api] demo: retry drop — reuse first placement's facing (yaw={target_yaw:.2f})")
        ros_bridge.rotate_by(diff)
    elif obj_x is not None and obj_y is not None:
        print(f"[robot_api] re-aligning after arm raise to face '{target}' ({obj_x:.2f}, {obj_y:.2f})")
        ros_bridge.rotate_to_face(obj_x, obj_y)
        if is_trash and attempt == 0:
            _trash_drop_yaw[target] = ros_bridge.get_current_orientation()
    ...
    # demo 調整：垃圾桶第二次放置（重試）在 drop 前再多前進一點，讓方塊更貼近桶口。
    if is_trash and not _is_manual_mode() and attempt == 1:
        print("[robot_api] demo: retry drop on trash — drive_forward extra 0.05m right before drop")
        ros_bridge.drive_forward(0.05)
    ...
    # laser 會掃到自己手臂寫進 costmap 變幻影障礙，下一次 nav 就 TEB state=4。
    # demo 調整：垃圾桶類目標放完不收手臂，保持 ARM_PUT（畫面上手臂伸進
    # 垂圾桶比較好看）。重試時 pick_up_obj 會走「原地重拾」不導航，
    # 不會有 TEB 幻影障礙問題。
    if is_trash:
        print("[robot_api] trash target — keeping arm in ARM_PUT (not restoring to ARM_CLAMP)")
    else:
        print("[robot_api] restoring arm to ARM_CLAMP after put")
        _set_arm_direct(ARM_CLAMP_POSE, secs=1.2)
