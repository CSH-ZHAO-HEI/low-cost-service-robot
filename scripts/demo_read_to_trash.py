#!/usr/bin/env python3
"""
Demo：红色方块 → 垃圾桶 (纯展示，跑一次)

特点：
  1. 互动输入界面 — 支持中文 / 英文指令，例如：
       "把红色方块丢到垃圾桶" 或 "Put the red block in the trash can"
  2. 完整输出 — LLM 生成计划、规则层 Judge、VLM Judge 原始输出、AdjustLLM
     微调代码全部即时打印，不隐藏任何区块。
  3. 红块起始位置在 NightStand_01_002 旁（朝蓝块方向偏移），离 Trash_01_001
     较远，放置时容易因定位/手臂误差让方块没有真正落入垃圾桶 →
     VLM Judge 判定失败 → AdjustLLM 重新拾取 → 再次放入垃圾桶。

用法：
  python3 scripts/demo_read_to_trash.py
"""
...
C_EXPERIMENT_DIR = os.path.join(PROJECT_ROOT, "experiments", "C1-C4")
sys.path.insert(0, BIG_BRAIN_DIR)
sys.path.insert(0, C_EXPERIMENT_DIR)
sys.path.insert(0, HERE)


RED_KEYWORDS = ["红色方块", "紅色方塊", "红方块", "紅方塊", "red block", "red_block", "red cube"]
TRASH_KEYWORDS = ["垃圾桶", "垂圾桶", "垃圾筒", "trash", "garbage", "bin"]

CANONICAL_INSTRUCTION = "Pick up the red_block and put it in Trash_01_001"
...
def parse_instruction(raw: str) -> str:
    ...
    has_trash = any(k.lower() in low for k in TRASH_KEYWORDS)
    if has_red and has_trash:
...
def setup():
    ...
    # Trash_01_001 在 gazebo_scene.yaml 有 4 个 approach_candidates (E/W/N/S)。
    # demo 固定使用东侧 (E) 的 canonical approach，不自动挑最近边。
    ...
    try:
        ...
        while True:
            raw = input(
                ...
                "例如：把红色方块丢到垃圾桶 / Put the red block in the trash can\n> "
            ).strip()
            ...
            try:
                ...
                trash = ros_bridge.get_obj_info("Trash_01_001")
                dx = info["object_x"] - trash["object_x"]
                dy = info["object_y"] - trash["object_y"]
                dist = math.hypot(dx, dy)
                check_text = "已落入垃圾桶 IN TRASH" if dist < comp.TRASH_NEAR_TOL else "MISSED"
